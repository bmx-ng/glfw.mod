...
Local app_name:String = "Camera Mouse Zoom"
...
Global yaw:Float = -90 ' yaw is initialized to -90.0 degrees since a yaw of 0.0 results in a direction vector pointing to the right so we initially rotate a bit to the left.
Global pitch:Float = 0.0
Global lastX:Float = 800.0 / 2.0
Global lastY:Float = 600.0 / 2.0
Global fov:Float = 45.0
...
Type TGameWindow Extends TGLFWWindow
	...
	Method OnCursorPosition(x:Double, y:Double)
		...
		Local sensitivity:Float = 0.1 ' change this value To your liking
		xoffset :* sensitivity
		yoffset :* sensitivity
		...
		Local front:SVec3F = New SVec3F(Float(Cos(yaw) * Cos(pitch)), Float(Sin(pitch)), Float(Sin(yaw) * Cos(pitch)))
...
Local ourShader:TShader = New TShader("7.3.camera.vs", "7.3.camera.fs")
