...
Local OutGladName$="glfwopengl.bmx"
...
Type TCodeGenerator
	...
	Method Init()
		gladSource = LoadText("../glfw.mod/glfw/deps/glad/gl.h").Split("~n")
	End Method
	...
	Method Generate()
		...
		WriteLine OutGlad	, "Function gladLoadGL:Int(func(name:Byte Ptr))=~qint gladLoadGL( GLADloadfunc)!~q"
		WriteLine OutGlad	, "End Extern"
		OutGlad.Close
	...
	Method Process()
		...
		For Text:String = EachIn gladSource
			bump
			If curr="GLAD_API_CALL"
				bump
				Local ty:Tty = New Tty
				Local funty$=gltype(ty)

				If funty<>"x" And curr="GLAD_API_PTR"
					Local id$=bump()
					If id[..2]="gl" And bump()="("
			...
			Else If curr="typedef"
				...
				Local ty:Tty = New Tty
				Local funty$=gltype(ty)
				If funty<>"x" And curr="(" And bump()="GLAD_API_PTR"
					Local id$=bump()
					If id = "*" Then id = bump()
	...
	Method HeaderOpenGL()
		...
		WriteLine OutOpenGL, "Import ~q../glfw.mod/glfw/deps/*.h~q"
		WriteLine OutOpenGL, "Import ~q../glfw.mod/glfw/deps/glad/gl.h~q"
	...
	Method HeaderGlad()
		...
		WriteLine OutGlad, ""
		WriteLine OutGlad, "Import ~q../glfw.mod/glfw/deps/*.h~q"
		WriteLine OutGlad, "Import ~q../glfw.mod/glfw/deps/glad/gl.h~q"
		WriteLine OutGlad, ""
		WriteLine OutGlad, "Extern"
		WriteLine OutGlad, ""
	End Method
	...
	Method MainGlad()
		...
		WriteLine OutGlad, "Module GLFW.GLFWOpenGL"
		WriteLine OutGlad, ""
